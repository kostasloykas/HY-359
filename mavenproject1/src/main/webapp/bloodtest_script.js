...
function req_new() {
    ...
    xhr.onload = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
            document.getElementById("msg-submit-bloodtest").innerHTML = "Καταχωρηθηκε με επιτυχια το Test";
            var amka = document.getElementById("amka").value;
            document.getElementById("myform").reset();
            document.getElementById("amka").value = amka;

            setTimeout(function () {
                document.getElementById("msg-submit-bloodtest").innerHTML = '';
            }, 2000);
...
function value1(measure, obj, i) {
    if (measure === "blood_sugar")
        return obj[i].blood_sugar;
    if (measure === "iron")
        return obj[i].iron;
    if (measure === "cholesterol")
        return obj[i].cholesterol;
    if (measure === "vitamin_d3")
        return obj[i].vitamin_d3;
    if (measure === "vitamin_b12")
        return obj[i].vitamin_b12;
}

function value2(measure, obj, i) {
    if (measure === "blood_sugar")
        return obj[i].blood_sugar_level;
    if (measure === "iron")
        return obj[i].iron_level;
    if (measure === "cholesterol")
        return obj[i].cholesterol_level;
    if (measure === "vitamin_d3")
        return obj[i].vitamin_d3_level;
    if (measure === "vitamin_b12")
        return obj[i].vitamin_b12_level;
}
